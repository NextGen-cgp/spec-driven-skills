# Example Contract Test Plan: Cancelled Parameter
...
| CT-004 | Reopen parameter | Valid cancelled parameter | Valid IDs | 200, status `PENDING`, `includedInGlobalResult=true` | integration | AC-004 |
| CT-005 | Get analysis detail | Cancelled parameter exists | analysisId | Response includes parameter status `CANCELLED` | integration/e2e | AC-003 |
| CT-006 | Complete analysis | Only cancelled parameters unfilled | analysisId | Completion allowed if remaining required parameters are valid | integration/e2e | BR-002 |

## 3. Handoff to test skill
